...
const LEAK_POOL  = "ze1HvkHogbWPRiR6W5DYp82YrtJTAum1WEDLrUJNjwX";
const SLIPPAGE   = 100; // 1 %

// SDK hardcodes TOKEN_PROGRAM_ID for tokenQuoteProgram even when quote is Token-2022.
function patchToken2022(tx: Transaction): Transaction {
...
async function sendAndConfirm(conn: Connection, tx: Transaction, wallet: WalletProvider): Promise<string> {
...
function fmt(lamports: string | number, decimals = 9) {
  return (Number(lamports) / 10 ** decimals).toFixed(4);
}
...
export default function SwapWidget({ dontLeakPoolAddress, dontLeakMint }: Props) {
  ...
  async function handleSwap() {
    ...
    try {
      ...
      // ── Step 2: DBC LEAK → DontLeak ──────────────────────────────────────
      addLog("Building DBC swap tx (LEAK → DontLeak)…");
      const client = DynamicBondingCurveClient.create(conn, "confirmed");

      // swapBaseForQuote: false = paying quote (LEAK) to receive base (DontLeak)
      const minOut = (leakOut * BigInt(10000 - SLIPPAGE * 2)) / BigInt(10000);
      const rawDbcTx = await client.pool.swap({
        owner:              wallet.publicKey,
        pool:               new PublicKey(dontLeakPoolAddress),
        amountIn:           new BN(leakOut.toString()),
        minimumAmountOut:   new BN(minOut.toString()),
        swapBaseForQuote:   false,
        referralTokenAccount: null,
      });
      const dbcTx = patchToken2022(rawDbcTx);

      addLog("Sign tx 2/2 — LEAK → DontLeak…");
      const sig2 = await sendAndConfirm(conn, dbcTx, wallet);
      addLog(`✓ DontLeak received  (${sig2.slice(0, 16)}…)`);
      setDone(sig2);
    } catch (e: unknown) {
  ...
  const accentCls  = isLeak ? "green" : "red";
  ...
  return (
    <div className={`rounded-2xl border bg-[#13131a] p-5 ${isLeak ? "border-green-500/20" : "border-red-500/20"}`}>
      ...
      <div className="mb-4 p-3 rounded-lg bg-white/3 border border-white/6 text-xs font-mono space-y-1">
        ...
        <div className="flex justify-between">
          <span className="text-white/30">LEAK out</span>
        ...
        {!isLeak && (
          <div className="flex justify-between">
            <span className="text-white/30">DontLeak out</span>
            <span className="text-red-400/70">~market rate</span>
          </div>
        )}
        {!isLeak && (
          <div className="pt-1 text-white/20 text-[10px]">
            2 txs: Jupiter SOL→LEAK, then Meteora LEAK→DontLeak
          </div>
        )}
      </div>
      ...
      {done && (
        <div className="mb-3 px-3 py-2 rounded-lg bg-green-500/10 border border-green-500/20 text-green-400 text-xs">
          ✓ Done! <a href={`https://solscan.io/tx/${done}`} target="_blank" rel="noopener noreferrer" className="underline opacity-70">View tx</a>
        </div>
      )}
